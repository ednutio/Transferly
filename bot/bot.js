...
function buildProviderOpsKeyboard(ctx, service, options = {}) {
  const keyboard = new InlineKeyboard();
  const hasInvoices = getProviderLaneStatus(service.slug, "invoices") === "live";
  const hasPayouts = getProviderLaneStatus(service.slug, "payouts") === "live";
  const hasBalance = getProviderLaneStatus(service.slug, "wallet-balance") === "live";
  const hasActivity = getProviderLaneStatus(service.slug, "provider-activity") === "live";
...
async function handleProviderWorkspace(ctx, slug) {
  ...
  clearPendingPrompts(ctx);
  const [statusPayload, featuresPayload] = access.isAdmin
    ? await Promise.all([
...
async function handleProviderBalance(ctx, slug) {
  ...
  const status = getProviderLaneStatus(service.slug, "wallet-balance");
  if (status !== "live") {
    await replyHtml(
      ctx,
      [
  ...
  const payload = await adminGet(`/api/admin/payment-providers/${service.slug}/balance`);
...
async function handleProviderWebhooks(ctx, slug) {
  ...
  const payload = await adminGet("/api/admin/webhooks", { provider: service.slug, limit: 12 }).catch(() => ({ data: [] }));
  const events = dataList(payload, "events", "items");
  const providerEvents = events.filter((event) => belongsToProvider(event, service.slug, eventProviderLabel));
  const visibleEvents = (providerEvents.length ? providerEvents : events).slice(0, 8);
  const lines = [
    `<b>${escapeHtml(service.title)} · Webhooks</b>`,
    "",
    line("Matched", providerEvents.length || "recent events"),
    "",
...
async function handleProviders(ctx) {
  ...
  if (access.isAdmin) {
    const payload = await adminGet("/api/admin/payment-providers").catch(() => null);
    const providers = dataList(payload, "providers", "items");
    lines.push(
      "",
      "<b>Backend Providers</b>",
      ...(providers.length ? providers.slice(0, 8).map(formatProvider) : ["Provider status data is not available from the API right now."]),
    );
  }
